import COLORS from '../styles/colors'
export default {
  aboutVKH:
    'Vysokoškolské Katolické Hnutí Ostrava je společenství mladých lidí, kteří studují vysokou školu nebo nedávno absolvovali. Společně těžíme ze sebe to nejlepší! Mladým lidem nabízíme kromě společenství možnost duchovního růstu, ale také čas pro sebevzdělávání či zábavu. Společně se modlíme, zpíváme, bavíme, vyrážíme do přírody či za kulturou. Pokud máš čas, neváhej a dojdi na Studentskou mši svatou do kostela Sv. Cyrila a Metoděje v Ostravě – Pustkovci. Scházíme se každé úterý od 19:00.',
  pray: {
    title: 'Duchovní program',
    text:
      'Jednou z hlavních náplní našeho společenství je prohlubování vztahu s Bohem. Pravidelně můžete navštěvovat studentskou mši svatou, pořádáme také adorace, modlitby růžence a v postní době se společně vydáváme na duchovní obnovu. Kdykoli je vám také k dispozici náš studentský kaplan o. Tomáš Čirž.',
    color: COLORS.EVENTS.PRAY,
  },
  fun: {
    title: 'Zábavné akce',
    text:
      'Je čas modlit se, ale je také čas se bavit! A na to my nezapomínáme. Kromě nejrůznějších výletů do přírody či za kulturou chodíme hrát fotbal nebo volejbal. Pravidelně měříme své síly v bowlingu a tužíme své vztahy při podzimním koštu pálenek! Je toho mnohem víc, stačí se přidat!',
    color: COLORS.EVENTS.FUN,
  },
  talk: {
    title: 'Přednášky',
    text:
      'Osobní rozvoj mladých je důležitý. Proto se snažíme k rozvoji přispívat zajímavými hosty, kteří mají co nabídnout. Navštívili nás hosté jako o. Miroslav Herold, dr. Maria Fridrichová nebo pomocný biskup naší diecéze o. Martin David. ',
    color: COLORS.EVENTS.TALK,
  },
  quotes: [
    'VKH je pro mě společenství mladých věřících studentů, mezi kterými se cítím přijatý.',
    'Do VéKaHáčka chodím, protože chci vytěžit ze sebe to nejlepší jak při darování krve, tak ve schole.',
    'Mše, modlitba, kamarádi, akce, zajímavé přednášky, "pivo" po programu - to je pro mě VKH.',
  ],
}
